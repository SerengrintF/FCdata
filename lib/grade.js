...
function fetchGradeInfo(ouid, userInfo, matchDetails, response) {
    ...
    fetchMaxDivisionGrade(ouid, (error, maxDivisionData) => {
        ...
        if (maxDivisionData && Array.isArray(maxDivisionData) && maxDivisionData.length > 0) {
            ...
            // matchType 50에 해당하는 등급만 필터링
            const matchType50Data = maxDivisionData.filter(item => 
                item.matchType === 50 || item.matchtype === 50
            );
            
            
            // matchType 50에서 다양한 필드명으로 division ID 추출 시도
            const divisionIds = matchType50Data.map(d => 
            ...
            if (divisionIds.length > 0) {
                ...
                // matchType 50 데이터가 있는지 확인
                if (matchType50Data.length === 0) {
                } else {
                }
            }
        } else {
