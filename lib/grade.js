...
function fetchGradeInfo(ouid, userInfo, matchDetails, response, matchType = 50) {
    ...
    fetchMaxDivisionGrade(ouid, (error, maxDivisionData) => {
        ...
        console.log(`[등급 처리] 매치코드: ${matchType}`);
        console.log(`[등급 처리] 전체 데이터:`, maxDivisionData);
        ...
        if (maxDivisionData && Array.isArray(maxDivisionData) && maxDivisionData.length > 0) {
            ...
            console.log(`[등급 처리] 필터링된 데이터 (매치코드 ${matchType}):`, matchTypeData);
            ...
            console.log(`[등급 처리] 추출된 division ID들:`, divisionIds);
            ...
            if (divisionIds.length > 0) {
                ...
                console.log(`[등급 처리] 최종 등급:`, maxDivision, maxDivisionInfo);
